	...
	Opa5.createPageObjects({
		onTheAppPage: {
			actions: {
				...
				iPressTheSayHelloButton: function () {
					return this.waitFor({
						...
						viewName: sViewName,
						actions: new Press(),
						errorMessage: "Did not find the 'Say Hello' button on the HelloPanel view"
					});
				}
			},
			...
			assertions: {
				...
				iShouldSeeTheHelloToast: function () {
					return this.waitFor({
						autoWait: false,
						check: function () {
						...
						success: function () {
							// we set the view busy, so we need to query the parent of the app
							Opa5.assert.ok(true, "The toast is visible");
						},
						errorMessage: "Toast not shown"
					});
				}
			}
		}
